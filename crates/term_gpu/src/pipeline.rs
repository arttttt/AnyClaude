//! Build the two render pipelines: `prim` (rectangle instances) and `text`
//! (glyph instances sampling the atlas). Both take the same `Uniforms` bind
//! group at @group(0); `text` additionally takes the atlas texture+sampler
//! at @group(1).

use crate::instances::{GlyphInstance, RectInstance};
