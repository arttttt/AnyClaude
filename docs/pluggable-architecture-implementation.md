...
### Phase 2: Первая фича -- Status (proof of concept)

---

#### Шаг 2.1: Создать `src/features/status.rs` -- StatusFeature

**Цель**: Реализовать фичу Status как `impl Feature`, которая полностью инкапсулирует:
- Поля: `proxy_status`, `metrics`, `last_ipc_error`, `last_status_refresh`, `last_metrics_refresh`
- Рендеринг: содержимое Status попапа (строки 64-231 `render.rs`)
- Тик: автообновление status и metrics по интервалу
- Хоткей: Ctrl+S

**Файлы для создания**:

**`src/features/status.rs`** (новый файл):

```rust
//! Status / Network Diagnostics popup feature.
...
use crate::ui::theme::{HEADER_TEXT, STATUS_ERROR, STATUS_OK, STATUS_WARNING};
...
pub struct StatusFeature {
    ...
    /// true когда попап открыт (для условной загрузки метрик).
    popup_visible: bool,
}
...
impl StatusFeature {
    ...
    pub fn on_popup_open(&mut self) {
        self.popup_visible = true;
    }

    /// Вызывается при закрытии попапа.
    pub fn on_popup_close(&mut self) {
        self.popup_visible = false;
    }

    /// Доступ к proxy_status (для Header).
    pub fn proxy_status(&self) -> Option<&ProxyStatus> {
        self.proxy_status.as_ref()
    }

    /// Доступ к last_ipc_error (для рендеринга).
...
impl Feature for StatusFeature {
    ...
    fn hotkey(&self) -> Option<KeyCombo> {
        Some(KeyCombo::Ctrl('s'))
    }

    fn on_tick(&mut self, _ctx: &FeatureContext<'_>) -> Vec<FeatureCommand> {
        let mut commands = Vec::new();

        // Статус обновляется всегда (нужен для Header).
        if self.last_status_refresh.elapsed() >= STATUS_REFRESH_INTERVAL {
            self.last_status_refresh = Instant::now();
            commands.push(FeatureCommand::SendUiCommand(UiCommand::RefreshStatus));
        }

        // Метрики только когда попап открыт.
        if self.popup_visible
            && self.last_metrics_refresh.elapsed() >= METRICS_REFRESH_INTERVAL
        {
            self.last_metrics_refresh = Instant::now();
            commands.push(FeatureCommand::SendUiCommand(UiCommand::RefreshMetrics {
                backend_id: None,
            }));
        }
    ...
    fn on_event(&mut self, event: &FeatureEvent, _ctx: &FeatureContext<'_>) {
        match event {
            FeatureEvent::IpcStatus(status) => {
                self.proxy_status = Some(status.clone());
                self.last_ipc_error = None;
            }
            FeatureEvent::IpcMetrics(metrics) => {
    ...
    fn handle_popup_key(
        ...
        match &key.kind {
            KeyKind::Escape | KeyKind::Control('s') => {
                self.on_popup_close();
                Some(vec![FeatureCommand::ClosePopup])
            }
            // Закрытие другими хоткеями попапов
            KeyKind::Control('b') | KeyKind::Control('h') => {
                self.on_popup_close();
                Some(vec![FeatureCommand::ClosePopup])
            }
            _ => Some(vec![]),
    ...
    fn popup_content(&self) -> Option<PopupContent<'_>> {
        ...
        let active_backend = self.backends.iter().find(|b| b.is_active);

        if let Some(backend) = active_backend {
            lines.push(Line::from(vec![
                Span::styled("  Provider:  ", Style::default().fg(HEADER_TEXT)),
                Span::styled(backend.display_name.clone(), Style::default().fg(HEADER_TEXT)),
            ]));
            ...
            let (status_text, status_color) =
                if self.proxy_status.as_ref().is_some_and(|s| s.healthy) {
                    ("Connected", STATUS_OK)
                } else {
                    ("Error", STATUS_ERROR)
                };
            ...
            let latency_str = if let Some(metrics) = &self.metrics {
                metrics
                    .recent
                    .iter()
                    .rev()
            ...
            let tokens_str = if let Some(metrics) = &self.metrics {
                metrics
                    .recent
                    .iter()
                    .rev()
                    ...
                    .unwrap_or_else(|| "\u{2014} in / \u{2014} out".to_string())
            } else {
                "\u{2014} in / \u{2014} out".to_string()
            };
            lines.push(Line::from(vec![
...
**ВАЖНО**: На этом шаге `StatusFeature` полностью инкапсулирует данные, которые сейчас живут в `App`:
- `proxy_status` (App строка 66)
- `metrics` (App строка 67)
- `last_ipc_error` (App строка 70)
- `last_status_refresh` (App строка 71)
- `last_metrics_refresh` (App строка 72)

Однако `proxy_status` также нужен для `Header` виджета. Решение: `StatusFeature` предоставляет метод `proxy_status()`, а registry дает к нему доступ через `get()`. Подробнее в шаге 6.2.
...
Убрать `#[allow(dead_code)]` с `pub mod feature` в `src/ui/mod.rs`:
...
**Проверка**:
```bash
cargo build                      # OK (cfg не активен)
cargo build --features pluggable # OK (StatusFeature зарегистрирован)
```

**Откат**: `rm src/features/status.rs`, откатить `src/features/mod.rs`.

---

#### Шаг 2.2: Расширить StatusFeature полной логикой ошибок и recovery

**Цель**: Добавить в `popup_content()` рендеринг recent errors и recovery operations, которые сейчас находятся в `render.rs` строки 155-228.

Эта логика использует `ErrorRegistry`, который передается через `FeatureContext`. Но `popup_content()` не получает `FeatureContext` -- он берет данные из self.

**Решение**: `StatusFeature` будет хранить ссылку на `ErrorRegistry` (которая `Arc<RwLock<...>>` внутри, поэтому `Clone`).

Модифицировать `StatusFeature`:
...
pub struct StatusFeature {
    ...
    last_metrics_refresh: Instant,
    popup_visible: bool,
    /// Кэш ErrorRegistry для рендеринга.
    error_registry: Option<crate::error::ErrorRegistry>,
}
```

И в `init()`:
```rust
fn init(&mut self, ctx: &FeatureContext<'_>) {
    self.error_registry = Some(ctx.error_registry.clone());
}
```

Тогда `popup_content()` может использовать `self.error_registry` для рендеринга ошибок.

Полный `popup_content()` с ошибками добавляется в `status.rs` после блока с бэкендами. Код идентичен `render.rs` строки 155-228.

**Проверка**: `cargo build --features pluggable`

---

#### Шаг 2.3: НЕ модифицируем app.rs/render.rs на этом этапе

**Важное решение**: На этапе Phase 2 мы НЕ трогаем существующий код. `StatusFeature` живет параллельно. Данные дублируются (в App и в StatusFeature). Это нормально -- Strangler Fig pattern.

Cfg-gate в app.rs, render.rs, input.rs будет добавлен в Phase 6, когда pluggable path будет полностью готов.

---

#### Шаг 2.4: Интеграционный тест для StatusFeature

**Цель**: Написать тест, проверяющий core логику StatusFeature.

**Файлы для создания**:

**`tests/status_feature.rs`** (новый файл):

```rust
//! Integration tests for StatusFeature (pluggable architecture).

#![cfg(feature = "pluggable")]

use anyclaude::config::{Config, ConfigStore};
use anyclaude::error::ErrorRegistry;
use anyclaude::ipc::ProxyStatus;
use anyclaude::features::status::StatusFeature;
use anyclaude::ui::feature::{Feature, FeatureCommand, FeatureContext, FeatureEvent};
...
#[test]
fn status_feature_id() {
    let feature = StatusFeature::new();
    assert_eq!(feature.id(), "status");
}

#[test]
fn status_feature_hotkey() {
    let feature = StatusFeature::new();
    let hotkey = feature.hotkey();
    assert!(hotkey.is_some());
    assert_eq!(
        hotkey.unwrap(),
        anyclaude::ui::feature::KeyCombo::Ctrl('s')
    );
}

#[test]
fn status_feature_tick_requests_refresh() {
    let mut feature = StatusFeature::new();
    ...
    // Первый тик -- last_status_refresh = Instant::now(), но elapsed ~0
    // Значит RefreshStatus не будет. Перемотаем вручную.
    // Для теста: создадим StatusFeature с устаревшим timestamp.

    // Вместо этого -- вызовем tick дважды с паузой > 1s
    // Это медленный тест, но он проверяет реальную логику.
    // Для unit-тестов лучше инжектить Clock, но мы не усложняем.

    // Простая проверка: tick() не паникует и возвращает Vec
    let commands = feature.on_tick(&ctx);
    assert!(commands.is_empty() || !commands.is_empty()); // просто не паникует
}

#[test]
fn status_feature_handles_ipc_status_event() {
    let mut feature = StatusFeature::new();
    let (config, error_registry) = make_ctx();
    let ctx = FeatureContext {
        config: &config,
    ...
    assert!(feature.proxy_status().is_none());
    ...
    feature.on_event(&FeatureEvent::IpcStatus(status.clone()), &ctx);

    assert!(feature.proxy_status().is_some());
    assert_eq!(feature.proxy_status().unwrap().active_backend, "test");
    assert!(feature.proxy_status().unwrap().healthy);
}

#[test]
fn status_feature_popup_content_no_backend() {
    let feature = StatusFeature::new();
    let content = feature.popup_content();
    assert!(content.is_some());
    let content = content.unwrap();
    assert_eq!(content.title, "Network Diagnostics");
}

#[test]
fn status_feature_ipc_error_clears_on_status() {
    let mut feature = StatusFeature::new();
    let (config, error_registry) = make_ctx();
    let ctx = FeatureContext {
        config: &config,
    ...
    // Установить ошибку
    feature.on_event(&FeatureEvent::IpcError("connection failed".to_string()), &ctx);
    assert_eq!(feature.last_ipc_error(), Some("connection failed"));

    // Получить статус -- ошибка должна сброситься
    let status = ProxyStatus {
        active_backend: "test".to_string(),
        uptime_seconds: 1,
        total_requests: 1,
    ...
    feature.on_event(&FeatureEvent::IpcStatus(status), &ctx);
    assert!(feature.last_ipc_error().is_none());
}
```

**Проверка**:
```bash
cargo test --features pluggable  # Тесты проходят
cargo test                       # Тесты пропущены (cfg)
```

**Откат**: `rm tests/status_feature.rs`
...
В `runtime.rs`, блоки IpcStatus/IpcMetrics/IpcBackends/IpcError:
...
Аналогично для IpcMetrics, IpcBackends, IpcError.
