...
> - **§6 mouse reporting (2026-05-30) — the last §6 item, now done.** `term_gpu::encode_mouse_x10` (`CSI M Cb Cx Cy`) + `encode_mouse_sgr` (`CSI < Cb ; Cx ; Cy M|m`) forward press/release/wheel to the PTY when an app turns reporting on; the coordinator encodes the cell under the cursor and threads it as `Msg.mouse_report`, and `apply` emits `Effect::WriteToPty` (suppressing selection / scroll). Spec-pinned by 7 exact-byte tests — the verification, since Claude Code is keyboard-driven and never enables mouse mode (no consumer to eyeball). Scoped to left press/release + wheel; term_core's single `MouseMode` enum collapses report-level + encoding (documented simplifications for a Claude-Code-only emulator). **Phase E is now complete.**
