...
> Status: **RATIFIED (2026-05-29).** The invariants R1–R15 (§1) are binding; every term_ui commit + review checks against them. **Implementation status:** Phase A ✅ (`159e3aa` — engine core + the R4 property gate) · Phase B ✅ (`816dba4`/`7d3c62f`/`3d4af45` + ticker fix `f7d9dfe` — the coordinator pattern, proven in `crates/term_ui/examples/coordinator.rs`) · Phase C ✅ chrome views (`135fcb2`/`2e64e82`/`360dafa`/`1dcf6f2` — a new domain-agnostic `uikit` crate: `Segment` + `header_bar`/`footer_bar` over term_ui, fed by the `ui::chrome_labels` presenter in anyclaude `src/`; proven in `examples/chrome_preview.rs`, user-verified. The original §15 sketch's RichRow widget proved unnecessary — plain `Stack`/`Text`/`Spacer`/`Block` sufficed — and the session-click hitbox + scroll/momentum are deferred to the real coordinator that replaces `GpuApp`) · Phase D ✅ popup state cutover (`2a761dd`/`699b8d7`/`dcdadad` — the 3 popups (backend-switch / history / settings) off MVI to plain `apply(&mut self, intent)` state machines, the Stores/Actors deleted, `GpuApp` migrated, `use mvi::Store` gone; `mvi` is now used only by the dead `src/ui/pty/*`, deleted with the crate in Phase F; user-verified live. Render kept immediate (`draw_*_popup`); term_ui-view-ification of popups + opacity transitions deferred to E with the coordinator. No TextField/caret needed — popups are nav+toggle, no text input) · Phase F ✅ MVI deleted (`921a55c` dead `src/ui/pty/*` PtyActor + tests, `2099052` the `crates/mvi` crate — done early since D left mvi with only the dead pty consumer; `grep -r mvi` empty, **R1 satisfied**) · **Phase E ✅ COMPLETE (2026-05-30)** — foundation (`9eabb62`→`7f3b7ac`: E.1 `term_geometry` pure layout/hit-test · E.2 `AppState` consolidates all bucket-1 truth · E.3 `input` R7 event-phase mapping · E.4 scroll/momentum reducer with effects-as-data), then E.5–E.6 chrome on a term_ui retained tree · E.7 all 3 popups on a second term_ui tree + open/close fade (`gpu/popup.rs` deleted) · E.8 the unified `Msg`/`apply`/`Effect` loop (R10 literal: every event → `dispatch` → pure `AppState::apply` → `perform_effects`) · §6 mouse-report encoders. `GpuApp` was dissolved IN PLACE into the coordinator (no separate swap step); the terminal grid stays a direct `populate_panel` full-emit (R5), NOT a retained view. **REFAC-6** then decomposed `app.rs` into `app/{mod,events,render,geometry,popups,clipboard,session_ops}.rs`. The whole migration is complete, user-verified, full suite green. All Rust is **illustrative sketch only** — it shows shape and intent, not final API. Warp citations are `path:line` under `/Users/artem/Projects/warp`. term_gpu/term_core citations are real signatures verified in `/Users/artem/Projects/ClaudeWrapper/crates/`. Adversarial review findings (four lenses) are resolved inline; each resolution is tagged **[Resolved: …]** or **[Rejected: …]**.
...
- **mouse-reporting-mode gate (as built, 2026-05-30).** The emulator exposes a `mouse_protocol() -> MouseProtocol` with the tracking level and the encoding as ORTHOGONAL fields (`grid.rs`): `tracking: {Off, Normal(1000), ButtonEvent(1002), AnyEvent(1003)}` × `encoding: {Default(CSI M), Sgr(1006)}`. Set by independent DECSETs, so `?1000h` + `?1006h` *compose* into click-tracking-with-SGR (the bug the old single conflated `MouseMode` enum had — enabling SGR clobbered the level). DECSET 9 (X10), 1005 (UTF-8) and 1015 (urxvt), plus folding modifier bits into `Cb`, are intentionally unsupported — all matching Warp (deprecated / SGR superseded them). 1004 focus is a separate flag (`CSI I`/`O`). The coordinator's per-event routing:

  ```rust
  // As built. proto = emulator.mouse_protocol(); the byte shape is decided by
  // term_gpu::encode_mouse_report / encode_motion_report (pure, byte-tested).
  if shift_held || !proto.is_active() { /* local selection / scroll */ }
  else {
      Msg::MouseReport(encode_mouse_report(button, kind, col, row, proto.is_sgr()))
      // press/release: left via MousePress, middle/right via MouseReport;
      // wheel = WheelUp/Down; motion (drag/move) via encode_motion_report
      // (1002 only while a button is held, 1003 always), deduped per cell.
  }
  ```
  Shift is the local-action bypass (a Shift-drag stays a text selection even under tracking — Warp does the same). The encoders are pure fns, unit-tested by exact bytes (19 cases) — the verification, since Claude Code is keyboard-driven and enables no mouse mode (no live consumer to eyeball). `MouseProtocol` set/reset is unit-tested too (7 compose / reset cases). **[Resolved — yagni-nodefer issue 4 (major): the draft gated on a boolean that discarded the level/encoding distinction; first built as a single conflated `MouseMode` enum (E.8 §6), then split into the orthogonal `MouseProtocol` here so a real app's level + encoding DECSETs compose.]**
...
**Scroll anchoring on new output (as built, 2026-05-30, user-verified).** `ScrollState.offset_y` is measured from the bottom (`0` = pinned). On new PTY output (`drain_pty`): if the view was at the bottom, re-pin to `0` (follow mode); if the user had scrolled up, **keep the visible lines fixed** by advancing `offset_y` by how much the buffer grew below *plus* how many lines were evicted off the top. Without the eviction term the view would drift one line per appended line once the 1000-line scrollback fills. `term_core`'s grid exposes a monotonic `lines_evicted()` counter (the analog of Warp's `num_lines_truncated`); the coordinator reads it before/after the drain. Mirrors Warp's content-anchored scroll for a from-bottom pixel model. (Before this, scrolled-up content drifted as output streamed.)
...
> **As built (2026-05-29, `2a761dd`→`dcdadad`, user-verified live).** Done as a **state-only cutover**, scoped tighter than the sketch above:
> - Each popup's MVI `Actor::handle_intent` became an inherent `apply(&mut self, intent)` on the state enum (in-place mutation; the `Intent` enum stays as the plain message vocabulary). `mvi::{State,Intent,Actor}` impls dropped; only `actor.rs` deleted — `state.rs`/`intent.rs`/`mod.rs` kept (minimal churn, less risk on the live `GpuApp`, vs the sketch's "delete state/intent/mod"). `GpuApp` holds three plain-state fields, `dispatch` → `apply`, and the now-unused `use mvi::Store` is gone. `mvi` survives only for the dead `src/ui/pty/*` → Phase F.
> - Tests ported off `Store`: `tests/backend_switch.rs` (+5), `tests/history.rs` (8, folds history_actor+state), `tests/settings.rs` (14).
> - **NOT done (deferred to E with the coordinator):** the term_ui-view-ification of the popups + opacity transitions + focus-trap-as-tree. The existing immediate `draw_*_popup` renders the new plain state for now — the same deferral C made for its hitbox/scroll, for the same reason (term_ui popup views want the real coordinator, not GpuApp's overlay). So E grew: it now also ports the popup views + transitions.
> - **No TextField/caret needed** — every popup is navigation + boolean toggle, no text input (YAGNI; §8 TextField stays unbuilt until a text-input consumer appears). **Latent bug noticed, left as-is:** the settings `RequestClose`/`confirm_discard` dirty-discard flow is unit-tested but NOT wired to the live Esc (`close_all_popups` dispatches `Close`); decide whether to wire it when the coordinator owns Esc routing in E.
...
> **In progress (2026-05-29, `9eabb62`→`7f3b7ac`, user-verified).** Done as a sequence of behaviour-preserving extractions that dissolve `GpuApp` toward R10 = `resources + one AppState + delegation to pure fns`, WITHOUT yet wiring term_ui views (GpuApp still renders immediate-mode). Each is GPU-free-testable:
> - **E.1** `src/ui/term_geometry.rs` — pure `terminal_panel_rect` / `fit_grid` / `cell_at` / `next_click_count` (plain `f32`, no `CellMetrics`). GpuApp delegates. 11 tests.
> - **E.2** `src/ui/app_state.rs` — `AppState` holds ALL bucket-1 truth (grid / scroll+momentum / selection / input / modifiers / session / 3 popups); GpuApp is now `state: AppState` + resources (split-brain gone). Copied-flash + uptime DERIVED (R12). 5 tests.
> - **E.3** `src/ui/input.rs` — pure R7 event-phase mapping (`app_shortcut` for Cmd-combos, `*_nav` for popup keys → intent). GpuApp handlers delegate. 6 tests.
> - **E.4** `AppState::{on_wheel, on_gesture_end, on_momentum_tick}` — pure scroll/momentum reducer returning `Vec<ScrollEffect>` (Elm-style effects-as-data); `GpuApp::perform_scroll_effects` runs the timers + redraw. +5 tests.
> - **E.5–E.6 (2026-05-30) — chrome on term_ui.** `chrome_labels::chrome_view(&AppState)` is built / reconciled / measured (tight window) / placed / painted into the overlay each frame (GpuApp gains `chrome_tree` / `chrome_root` / `chrome_prev` / `chrome_scratch`). The session-click hitbox (deferred from C) is restored via a stable `WidgetId` resolved from the laid-out tree (R7). Full-width separators come from "a `Block` stretches its single child" in the `place` pass. `gpu/chrome.rs` is slimmed to dimension constants.
> - **E.7 (2026-05-30) — all 3 popups on term_ui; `gpu/popup.rs` DELETED.** A SECOND retained tree (`popup_tree` / `popup_root` / `popup_prev: Block` / `popup_scratch`) + the `ui::popup_view` presenter (history + settings from `AppState`; backend via `backend_view(…)`, built by the coordinator because it needs the backend list + override ids `AppState` doesn't carry). Each popup is a `popup_box` `Block` (bg + 1px border + drop shadow + padding), measured under a `POPUP_MIN_WIDTH` floor and centred with the new `place_centered`, painted on top of chrome (its `Block` drop shadow flows through; the chrome path omits shadows). **New primitives landed first (E.7.0):** `BlockStyle.shadow` + `block_shadow` emit (the §11 Block-shadow capability, now real), `place_centered`, `term_ui::anim` (`ease_out`/`ease_in_out`/`lerp` + `apply_overlay_alpha`), `uikit::{popup_list, fixed_row_window}` (the §11 List widget + the §10 visible-range seam). History gains real R11 virtualization; settings Esc / click-outside now drive the `RequestClose` dirty-confirm — closing the latent bug §13/Phase-D flagged; open/close opacity fade (§9, R12) via a `ui::popup_anim` epoch + `apply_overlay_alpha` (fade-out keeps the retained tree alive until t≥1). Adversarially reviewed by a 25-agent workflow: **0 critical / 0 high**.
> - **Terminal grid (R5) — SUPERSEDES the "TerminalSurface view" in the Phase E sketch above.** The grid stays a DIRECT `populate_panel` full-emit each frame; it is NOT retained as a term_ui view (the emulator has no dirty API, so a retained grid would re-emit wholesale anyway). Only the chrome + popups — which have stable structure — are term_ui retained trees. And because `GpuApp` was dissolved IN PLACE into the coordinator, there is no separate "swap + delete `GpuApp`" step; only `gpu/popup.rs` was deleted (E.7) and `gpu/chrome.rs` slimmed (E.6).
> - **E.8 (2026-05-30) — the unified `Msg`/`apply`/`Effect` loop (the Phase-B coordinator pattern, finally live).** Every winit/user event now funnels through `GpuApp::dispatch` → `AppState::apply(Msg, &ApplyCtx) -> Vec<Effect>` → `GpuApp::perform_effects`. `apply` is the single, pure state-transition point; `perform_effects` is the single side-effect site (and returns `should_exit` for `Effect::Quit`, since the `ActiveEventLoop` is the coordinator's). The coordinator does the read-only resource resolution to build pure `Msg`s (resolve `cell_at`, read the backend list, put the emulator snapshot in `ApplyCtx`); resource WRITES come back as `Effect`s (`WriteToPty`, `ResizeEmulatorAndPty`, `Toggle*Popup`, `ApplyBackendSelection`, `SaveSettings`, `Copy*`, `Paste`, `RestartPty`, `DumpDiagnostic`, `Drain`, `Quit`, plus the scroll timers + `Redraw`). Only `RedrawRequested` stays direct (it IS the render). This makes R10 literal — `GpuApp` is `resources + one AppState + two term_ui trees + a pure apply/effects loop` — and supersedes the note above that there was "no separate `Msg`/`apply` loop". The per-event GpuApp wrappers (`on_wheel`/`on_gesture_end`/`on_momentum_tick`/`update_modifiers`/`on_cursor_moved`/`on_mouse_release`/`handle_popup_key`/`handle_*_key`/`request_close_popups`/`any_popup_visible`) are deleted; `apply` is headlessly tested per arm (28 `app_state` tests).
> - **§6 mouse reporting (2026-05-30; full coverage 2026-05-30).** First shipped as left-press/release + wheel over a single conflated `MouseMode` enum; then brought to FULL coverage (Warp-modelled). The model is now the orthogonal `MouseProtocol { tracking: {Off, Normal/1000, ButtonEvent/1002, AnyEvent/1003}, encoding: {Default, Sgr/1006} }` — set by independent DECSETs so level + encoding COMPOSE (the conflated enum let SGR clobber the level). `term_gpu::encode_mouse_report(button, kind, col, row, sgr)` is the one byte-shape authority (`MouseButton {Left,Middle,Right,WheelUp,WheelDown,None}` × `{Press,Release,Motion}` → legacy `CSI M` / SGR form); `encode_motion_report` adds the drag/move gating (1002 only with a button held, 1003 always — drag if held else bare `None`-button move, deduped per cell). The coordinator forwards left (via `MousePress`), middle/right (via `Msg::MouseReport`), wheel, and motion (via `Msg::CursorMoved.motion_report`), with Shift as the local-selection bypass; `apply` emits `Effect::WriteToPty`. **Deliberately omitted, matching Warp:** DECSET 9 (X10), 1005 (UTF-8) / 1015 (urxvt) encodings, and modifier bits in `Cb`. Focus reporting (1004) is a separate flag (`CSI I`/`O`). Spec-pinned by 19 exact-byte/gating tests (term_gpu) + 7 protocol compose/reset tests (term_core) — the verification, since Claude Code is keyboard-driven and enables no mouse mode (no consumer to eyeball). **Phase E is now complete.**
> - **Key encoding (full, 2026-05-30; user-verified live).** `encode_key(key, key_unmod, modifiers, app_cursor)` covers the xterm key table (Warp-modelled), expanded from a minimal first version that broke special keys: **Shift+Tab → `CSI Z`**; arrows / Home / End branch on DECCKM (`SS3 X` in application-cursor mode, else `CSI X`) for the plain form, and `CSI 1 ; <mod> X` when modified (`mod = 1 + shift + alt*2 + ctrl*4`, the ASCII digit; modified forms always use `CSI`, even under DECCKM); **Alt / Meta = `ESC` + the un-composed base char** (`key_unmod`, from winit `key_without_modifiers()` — on macOS this strips the Option composition so `Option+a` is `ESC a`, not `ESC å`); Ctrl+letter → C0 (`Ctrl+Alt` adds the ESC prefix), plus the non-letter Ctrl combos; F1–F4 `SS3 P/Q/R/S`, F5–F12 `CSI <n> ~`, both modifier-spliced; backspace editing combos `Opt → ESC DEL` (kill-word), `Ctrl → ^W` (kill-word), `Cmd → ^U` (kill-line, handled in `on_key`'s Super branch since Cmd is intercepted before the encoder). The coordinator resolves the two resource-backed inputs — `key_without_modifiers()` and the emulator's DECCKM state — into the pure `Msg::Key` (R10). **Deliberately omitted (matching Warp): application keypad mode and the Kitty keyboard protocol.** First tests for the encoder: `tests/key_encode.rs` (11 exact-byte cases). Unlike §6 mouse, this path HAS a live consumer (the user types), so it was user-verified before docs.

**Phase F ✅ DONE (2026-05-29, `921a55c` + `2099052`, user-verified).** Done EARLY — out of A→F order — because Phase D left `mvi` with a single consumer: the dead `src/ui/pty/*` MVI machine (`PtyActor` etc., never wired; the live PTY is the separate `gpu/pty.rs::ChildPty`, Option A). So F.1 deleted `src/ui/pty/*` + `tests/pty_actor.rs` (24) + `tests/pty_state.rs` (3); F.2 removed `crates/mvi` from the workspace + anyclaude's deps. `grep -r mvi src/ crates/` is empty (**R1 satisfied**); full `cargo test` green.
