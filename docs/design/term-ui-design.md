...
> - **Remaining (the render-heavy bulk):** the `TerminalSurface` view (needs a term_ui custom paint-leaf bridging `populate_panel`), selection reducer (needs the emulator snapshot), resize / `drain_pty` as `Msg`s, the popup views + transitions + focus-trap, the deferred-from-C chrome hitbox + scroll wiring, coordinator assembly, and the `main.rs` swap + deletion of `GpuApp`/`gpu/chrome.rs`/`gpu/popup.rs` (last, live-verified). Render-blind for the author → each piece is verified by running.
