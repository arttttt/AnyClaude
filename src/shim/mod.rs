...
impl TeammateShim {
    ...
    ///
    /// The shim forwards every tmux verb to the proxy's `/api/tmux` control
    /// plane (no real tmux). `session_token` is injected as
    /// `ANTHROPIC_CUSTOM_HEADERS` so teammate processes spawned via tmux
    /// `send-keys` can authenticate with the proxy. `session_id` names the
    /// per-session persistent log; `log_enabled` controls whether the shim
    /// writes to `tmux_shim.log`.
